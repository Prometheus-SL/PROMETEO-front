...
type SparkAccessoryRenderContext = {
  accessory: Accessory;
  eyeY: number;
  eyeXOffset: number;
  strokeColor: string;
  bodyGradientId: string;
  bodyGlowFilterId: string;
  crownGradientId: string;
  batGradientId: string;
  batGlowFilterId: string;
};

type AccessoryRenderer = (context: SparkAccessoryRenderContext) => ReactNode;

const renderNone: AccessoryRenderer = () => null;

const renderGlasses: AccessoryRenderer = ({ eyeXOffset, eyeY, strokeColor }) => (
  <g>
    <rect
      x={92 - eyeXOffset - 12}
      y={eyeY - 10}
...
const renderCrown: AccessoryRenderer = ({
  crownGradientId,
  bodyGlowFilterId,
}) => (
  <svg
  ...
  >
    ...
    <g>
      <g>
        <g>
          <path
            style={{ fill: `url(#${crownGradientId})` }}
...
const renderAntenna: AccessoryRenderer = ({ strokeColor }) => (
  <g>
    <line x1="92" y1="24" x2="92" y2="8" stroke={strokeColor} strokeWidth="3" />
    <circle
      cx="92"
      cy="6"
      r="4"
...
const renderBatman: AccessoryRenderer = ({
  batGradientId,
  batGlowFilterId,
  bodyGradientId,
}) => (
  ...
  >
    <defs>
      <linearGradient id={batGradientId} x1="0%" y1="0%" x2="100%" y2="100%">
    ...
    <g filter={`url(#${batGlowFilterId})`}>
      <path
        d="
          ...
          Z
        "
        fill={`url(#${batGradientId})`}
        strokeWidth="2"
        transform="translate(0, -40)"
      />
    </g>

    <g filter={`url(#${batGlowFilterId})`} transform="translate(115,130) scale(0.09)">
      <path
...
const renderTresCreus: AccessoryRenderer = () => (
  <image
    href={TresCreusSvg}
...
const renderSpiderman: AccessoryRenderer = () => (
  <image
    href={SpidermanSvg}
    role="img"
    xmlns="http://www.w3.org/2000/svg"
...
const ACCESSORY_RENDERERS: Record<Accessory, AccessoryRenderer> = {
  none: renderNone,
  ...
  trescreus: renderTresCreus,
};

export function renderSparkAccessory(context: SparkAccessoryRenderContext) {
  const renderer = ACCESSORY_RENDERERS[context.accessory] ?? renderNone;
