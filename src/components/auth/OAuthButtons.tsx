import { useCallback, useEffect, useRef, useState } from "react";
import { Button } from "@/components/ui/button";
import { Spinner } from "@/components/ui/spinner";
import { authService, type Tokens } from "@/services/auth";
import { useAuthContext } from "@/providers/AuthProvider";

type OAuthProvider = "google" | "github" | "discord";

function GoogleIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  );
}

function GitHubIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2z" />
    </svg>
  );
}

function DiscordIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
      <path
        d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"
        fill="#5865F2"
      />
    </svg>
  );
}

const PROVIDERS: {
  id: OAuthProvider;
  label: string;
  icon: typeof GoogleIcon;
}[] = [
  { id: "google", label: "Google", icon: GoogleIcon },
  { id: "github", label: "GitHub", icon: GitHubIcon },
  { id: "discord", label: "Discord", icon: DiscordIcon },
];

export default function OAuthButtons() {
  const [loadingProvider, setLoadingProvider] = useState<OAuthProvider | null>(
    null,
  );
  const [error, setError] = useState<string | null>(null);
  const popupRef = useRef<Window | null>(null);
  const popupTimerRef = useRef<number | null>(null);
  const { loginQR } = useAuthContext();

  // Keep a stable ref so the message handler never uses a stale closure
  const loginQRRef = useRef(loginQR);
  loginQRRef.current = loginQR;

  const clearPopupWatcher = useCallback(() => {
    if (popupTimerRef.current !== null) {
      window.clearInterval(popupTimerRef.current);
      popupTimerRef.current = null;
    }
  }, []);

  useEffect(() => {
    const channel = new BroadcastChannel("prometeo-oauth-login");

    channel.onmessage = async (event: MessageEvent) => {
      if (event.data?.type !== "oauth-login-callback") return;

      if (event.data?.callbackId) {
        channel.postMessage({
          type: "oauth-login-ack",
          callbackId: event.data.callbackId,
        });
      }

      clearPopupWatcher();
      popupRef.current = null;

      const data = event.data as {
        callbackId?: string;
        status?: string;
        tokens?: Tokens;
        error?: string;
      };

      if (data.status === "success" && data.tokens) {
        try {
          const { user } = await authService.getMe(data.tokens.accessToken);
          await loginQRRef.current(data.tokens, user);
          // Hard reload — tokens already in localStorage from loginQR
          window.location.replace("/");
        } catch {
          setError("Could not complete sign in. Try again.");
          setLoadingProvider(null);
        }
      } else {
        setError(data.error || "OAuth sign in failed. Try again.");
        setLoadingProvider(null);
      }
    };

    return () => {
      channel.close();
      clearPopupWatcher();
    };
  }, [clearPopupWatcher]);

  const handleOAuth = async (provider: OAuthProvider) => {
    if (loadingProvider) return;

    // Open about:blank immediately (synchronous with click — avoids popup blocker)
    const popup = window.open(
      "about:blank",
      `prometeo-oauth-${provider}`,
      "popup=yes,width=560,height=700",
    );

    popupRef.current = popup;
    setLoadingProvider(provider);
    setError(null);

    try {
      const { authorizeUrl } = await authService.getOAuthUrl(provider);

      if (popup && !popup.closed) {
        popup.location.href = authorizeUrl;
      } else {
        // Popup was blocked or closed — fall back to redirect
        window.location.href = authorizeUrl;
        return;
      }

      // Poll to detect if the user closed the popup manually
      clearPopupWatcher();
      popupTimerRef.current = window.setInterval(() => {
        if (!popupRef.current || popupRef.current.closed) {
          clearPopupWatcher();
          popupRef.current = null;
          setLoadingProvider(null);
        }
      }, 500);
    } catch (err) {
      setLoadingProvider(null);
      if (popup && !popup.closed) popup.close();
      setError(
        err instanceof Error
          ? err.message
          : "Could not start OAuth sign in. Try again.",
      );
    }
  };

  return (
    <div className="space-y-3">
      <div className="grid grid-cols-3 gap-2">
        {PROVIDERS.map(({ id, label, icon: Icon }) => (
          <Button
            key={id}
            type="button"
            variant="outline"
            size="sm"
            className="h-10 w-full border-white/10 bg-background/60 backdrop-blur hover:bg-muted/70"
            disabled={loadingProvider !== null}
            onClick={() => handleOAuth(id)}
            aria-label={`Sign in with ${label}`}
            title={`Sign in with ${label}`}
          >
            {loadingProvider === id ? (
              <Spinner className="size-4" />
            ) : (
              <Icon className="size-4" />
            )}
            <span className="sr-only">Sign in with {label}</span>
          </Button>
        ))}
      </div>
      {error && (
        <div
          role="alert"
          className="rounded-md border border-red-500/20 bg-red-500/5 px-3 py-2 text-xs text-red-600"
        >
          {error}
        </div>
      )}
    </div>
  );
}
